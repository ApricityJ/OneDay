...
from preprocess.prepare_data import calculate_age
# from preprocess.process_before_select import preprocess_all
...
def create_data_bunch_from_csv(category_cols: List = None) -> None:
    if category_cols is None:
        category_cols = []
    data_bunch_train = Bunch()

    train = loader.to_df(Path(dir_train).joinpath(f'train_all.csv'))
    test = loader.to_df(Path(dir_test).joinpath(f'test_all.csv'))

    # print(train.columns)
    # print(test.columns)
    # print(train.head(5))

    train['NTRL_CUST_AGE'] = train['NTRL_CUST_AGE'].apply(calculate_age)
    test['NTRL_CUST_AGE'] = test['NTRL_CUST_AGE'].apply(calculate_age)
    train['NTRL_CUST_SEX_CD'] = train['NTRL_CUST_SEX_CD'].apply(lambda x: 0 if x == 'A' else 1)
    test['NTRL_CUST_SEX_CD'] = test['NTRL_CUST_SEX_CD'].apply(lambda x: 0 if x == 'A' else 1)
    ...
    data_bunch_train.target = label_col
    data_bunch_train.data = train
    data_bunch_train.col_names = train.columns.tolist()
    data_bunch_train.category_cols = category_cols
    pickle.dump(data_bunch_train, open(Path(dir_train).joinpath(file_name_train), "wb"))

    data_bunch_predict = Bunch()
    data_bunch_predict.id = test_id_col
    data_bunch_predict.data = test
    pickle.dump(data_bunch_predict, open(Path(dir_test).joinpath(file_name_test), "wb"))
