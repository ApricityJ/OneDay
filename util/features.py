...
=========================
使用过程中调用Part 4进行批量自动化特征衍生
目标是能够区分执行训练集和测试集的特征衍生过程，
...
def Group_Statistics_Extension(colNames, keyCol, features):
    ...
    for col_temp in colNames:
        ...
        features_new[col_temp + '_norm_' + keyCol] = (features_new[keyCol] - features_new[col_mean]) / (
...
def timeSeriesCreation(timeSeries, timeStamp=None, precision_high=False):
    ...
    for timeStamp, timeStampName in zip(timeStamp_l, timeStamp_name_l):
        ...
        features_new['time_diff_months' + '_' + timeStampName] = np.round(
            features_new['time_diff_days' + '_' + timeStampName] / 30).astype('int')
        if precision_high != False:
...
def Cross_Combination(colNames,
    ...
    if multi == False:
        features_train_new, colNames_train_new = Binary_Cross_Combination(colNames=colNames, features=X_train,
                                                                          OneHot=OneHot)
        features_test_new, colNames_test_new = Binary_Cross_Combination(colNames=colNames, features=X_test,
                                                                        OneHot=OneHot)
    else:
        features_train_new, colNames_train_new = Multi_Cross_Combination(colNames=colNames, features=X_train,
                                                                         OneHot=OneHot)
        features_test_new, colNames_test_new = Multi_Cross_Combination(colNames=colNames, features=X_test,
                                                                       OneHot=OneHot)

    # 然后判断训练集和测试集的衍生特征是否存在差异
    if colNames_train_new != colNames_test_new:
        features_train_new, features_test_new, colNames_train_new, colNames_test_new = Features_Padding(
...
def Polynomial_Features(colNames,
    ...
    """
    if multi == False:
        features_train_new, colNames_train_new = Binary_PolynomialFeatures(colNames=colNames, degree=degree,
                                                                           features=X_train)
        features_test_new, colNames_test_new = Binary_PolynomialFeatures(colNames=colNames, degree=degree,
                                                                         features=X_test)
    else:
        features_train_new, colNames_train_new = Multi_PolynomialFeatures(colNames=colNames, degree=degree,
                                                                          features=X_train)
        features_test_new, colNames_test_new = Multi_PolynomialFeatures(colNames=colNames, degree=degree,
                                                                        features=X_test)
...
def timeSeries_Creation(timeSeries_train,
    ...
    :return features_new, colNames_new：返回创建的新特征矩阵和特征名称
    """
    features_train_new, colNames_train_new = features_new, colNames_new = timeSeriesCreation(
        timeSeries=timeSeries_train,
        timeStamp=timeStamp,
